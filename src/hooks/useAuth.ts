  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (loaded.length === 0) {
        ...
        // Try to sync with cloud - FIXED for cross-device same username
        if (navigator.onLine) {
          try {
            const cloudUsers = await cloudGetUsers();
            if (cloudUsers && cloudUsers.length > 0) {
              console.log(`☁️ Found ${cloudUsers.length} users in cloud, merging with ${loaded.length} local`);
              
              // NEW MERGE LOGIC: Cloud wins for same username to ensure same userId across devices
              // This fixes: Device A admin id X, Device B admin id Y (different) -> should use X (cloud) for both
              // So data saved per userId will be same across devices
              const mergedMap = new Map<string, User>(); // key = username lowercased
              // Add local users to map
              for (const u of loaded) {
                mergedMap.set(u.username.toLowerCase(), u);
              }
              // Cloud users overwrite local if same username (cloud is source of truth for id)
              ...
              for (const cu of cloudUsers) {
                ...
                if (!existing) {
                  ...
                  hasChanges = true;
                  console.log(`☁️ Adding new user from cloud: ${cu.username} (${cu.id})`);
                } else if (existing.id !== cu.id) {
                  // Same username but different id - use cloud's id to keep data consistent across devices
                  console.log(`☁️ Same username ${cu.username} has different ids: local ${existing.id} vs cloud ${cu.id} - using cloud id to sync data`);
                  mergedMap.set(key, cu);
                  hasChanges = true;
                } else {
                  // Same id, keep cloud version if newer (compare lastLogin or updated)
                  // For now, keep existing but could update
                }
                // Also check by email
                const emailKey = cu.email?.toLowerCase();
                if (emailKey && !mergedMap.has(emailKey)) {
                  // Ensure email not duplicate
                }
              }

              const merged = Array.from(mergedMap.values());
              
              // If merged has changes or cloud has admin that local also has but with different id, use merged
              if (hasChanges || merged.length !== loaded.length) {
                console.log(`☁️ Merged users: ${loaded.length} local + ${cloudUsers.length} cloud -> ${merged.length} total, using cloud ids for same usernames`);
                loaded = merged;
                saveUsers(loaded);
                
                // If current session was for local admin that got replaced by cloud admin with different id, update session
                const sess = loadSession();
                if (sess) {
                  const oldUser = loaded.find(u => u.id === sess.userId);
                  if (!oldUser) {
                    // Session id no longer exists (was replaced by cloud id), try find by username
                    const prevLocalAdmin = loadUsers().find(u => u.id === sess.userId);
                    if (prevLocalAdmin) {
                      const newUserWithSameUsername = merged.find(u => u.username.toLowerCase() === prevLocalAdmin.username.toLowerCase());
                      if (newUserWithSameUsername && newUserWithSameUsername.id !== sess.userId) {
                        console.log(`🔄 Updating session from old id ${sess.userId} to new cloud id ${newUserWithSameUsername.id} for same username`);
                        saveSession(newUserWithSameUsername.id);
                      }
                    }
                  }
                }
              } else if (loaded.length === 1 && loaded[0].username === 'admin' && cloudUsers.length === 1 && cloudUsers[0].username === 'admin' && loaded[0].id !== cloudUsers[0].id) {
                // Special case: both have single admin but different ids - use cloud
                console.log('☁️ Both have single admin with different ids - using cloud admin id for cross-device sync');
                loaded = cloudUsers;
                saveUsers(loaded);
              }
            }
