...
function EstimateForm({ estimate, customers, jobs, onSave, onClose }: { estimate: Estimate | null; customers: Customer[]; jobs: Job[]; onSave: (e: Estimate) => void; onClose: () => void }) {
  const [f, setF] = useState<Estimate>(estimate || { id: uuidv4(), customerId: '', customerName: '', jobId: undefined, title: '', jobType: 'residential_remove_replace', lineItems: [], subtotal: 0, taxRate: 0, tax: 0, total: 0, status: 'draft', validUntil: new Date(Date.now() + 30 * 24 * 3600 * 1000).toISOString().split('T')[0], notes: '', createdAt: new Date().toISOString(), });
  ...
  return (
    <Modal title={estimate ? 'Edit Estimate' : 'New Estimate - Job Type Specific AI'} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold mb-1">Customer (Optional - Can Add Later)</label><select value={f.customerId || ''} onChange={e => { const c = customers.find(x => x.id === e.target.value); setF(p => ({ ...p, customerId: e.target.value || '', customerName: c?.name || '', customerEmail: c?.email || p.customerEmail || '', customerPhone: c?.phone || p.customerPhone || '' })); }} className="w-full px-3 py-2 border-2 rounded-lg"><option value="">No Customer - Add Later</option>{customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>{!f.customerId && <p className="text-[10px] text-amber-600 mt-1">No customer now - you can create estimate/invoice without customer and assign later via Edit. AI can also do this: "Create estimate 1500 sq ft driveway" (no customer needed)</p>}</div><div><label className="block text-sm font-bold mb-1">Linked Job</label><select value={f.jobId || ''} onChange={e => setF(p => ({ ...p, jobId: e.target.value || undefined }))} className="w-full px-3 py-2 border rounded-lg"><option value="">No job linked</option>{jobs.filter(j => j.customerId === f.customerId || !f.customerId).map(j => <option key={j.id} value={j.id}>{j.title} ({j.squareFootage || '?'} sq ft)</option>)}</select></div></div>
        {!f.customerId && (
          <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-3 space-y-2">
            <p className="text-xs font-black text-amber-800">Optional Inline Customer Info (if you don't want to create customer record yet)</p>
            <div className="grid grid-cols-2 gap-2">
              <input value={f.customerName || ''} onChange={e=>setF(p=>({...p, customerName: e.target.value}))} placeholder="Customer Name (optional)" className="px-3 py-2 border rounded-lg text-sm" />
              <input value={f.customerPhone || ''} onChange={e=>setF(p=>({...p, customerPhone: e.target.value}))} placeholder="Phone (optional)" className="px-3 py-2 border rounded-lg text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input value={f.customerEmail || ''} onChange={e=>setF(p=>({...p, customerEmail: e.target.value}))} placeholder="Email (optional)" className="px-3 py-2 border rounded-lg text-sm" />
              <input value={f.customerAddress || ''} onChange={e=>setF(p=>({...p, customerAddress: e.target.value}))} placeholder="Address (optional)" className="px-3 py-2 border rounded-lg text-sm" />
            </div>
            <p className="text-[10px] text-gray-500">This info will show on print even without a customer record. Later you can assign a real customer via Edit → Customer dropdown.</p>
          </div>
        )}
...
function InvoiceForm({ invoice, customers, estimates, onSave, onClose }: { invoice: Invoice | null; customers: Customer[]; estimates: Estimate[]; onSave: (i: Invoice) => void; onClose: () => void }) {
  const [f, setF] = useState<Invoice>(invoice || { id: uuidv4(), customerId: '', customerName: '', title: '', jobType: 'residential_remove_replace', lineItems: [], subtotal: 0, taxRate: 0, tax: 0, total: 0, amountPaid: 0, balanceDue: 0, status: 'draft', dueDate: new Date(Date.now() + 30 * 24 * 3600 * 1000).toISOString().split('T')[0], createdAt: new Date().toISOString(), });
  ...
  return (
    <Modal title={invoice ? 'Edit Invoice - Job Type Specific' : 'New Invoice - Job Type Specific'} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3"><div><label className="text-sm font-bold">From Estimate (will copy job type too)</label><select onChange={e => loadFromEstimate(e.target.value)} className="w-full px-3 py-2 border-2 rounded-xl"><option value="">-- Select to import --</option>{estimates.map(e => <option key={e.id} value={e.id}>{e.title} - ${e.total.toFixed(0)} [{(e as any).jobType ? (e as any).jobType.replace(/_/g,' ') : 'no type'}]</option>)}</select></div><div><label className="text-sm font-bold">Customer (Optional - Can Add Later)</label><select value={f.customerId || ''} onChange={e => { const c = customers.find(x => x.id === e.target.value); setF(p => ({ ...p, customerId: e.target.value || '', customerName: c?.name || '', customerEmail: c?.email || p.customerEmail || '', customerPhone: c?.phone || p.customerPhone || '' })); }} className="w-full px-3 py-2 border-2 rounded-xl"><option value="">No Customer - Add Later</option>{customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>{!f.customerId && <p className="text-[10px] text-amber-600 mt-1">No customer now - can create without and assign later. AI: "Create invoice 1500 sq ft driveway" works without customer.</p>}</div></div>
        {!f.customerId && (
          <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-3 space-y-2">
            <p className="text-xs font-black text-amber-800">Optional Inline Customer Info</p>
            <div className="grid grid-cols-2 gap-2">
              <input value={f.customerName || ''} onChange={e=>setF(p=>({...p, customerName: e.target.value}))} placeholder="Customer Name (optional)" className="px-3 py-2 border rounded-lg text-sm" />
              <input value={f.customerPhone || ''} onChange={e=>setF(p=>({...p, customerPhone: e.target.value}))} placeholder="Phone (optional)" className="px-3 py-2 border rounded-lg text-sm" />
            </div>
            ...
            <p className="text-[10px] text-gray-500">Shows on print even without customer record. Assign real customer later via Edit.</p>
          </div>
        )}
